...
// ~/.claude/skills/ location — it installs to .agents/skills/ which Claude Code
// doesn't read. Direct fetch puts the file exactly where it needs to be.
async function fetchOfficialSkill(scope, dryRun) {
...
async function main() {
  ...
  // Step 1 — official skill (direct fetch from GitHub, not via `skills` CLI)
  if (!skipOfficial) {
    step("Installing official arkiv-best-practices skill (direct fetch from GitHub)...");
    const success = await fetchOfficialSkill(isProject ? "project" : "user", dryRun);
